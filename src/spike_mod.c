...
/* ── TEMPORARY save-path diagnostic (delete once the location is known) ─────
 ...
 * mod can send a `save` message, this one-shot probe finds the game's own copy
 * of the loaded save's path/name. It dumps the head of the save object and
 * searches the object (and up to SAVE_DIAG_MAX_POINTERS of its first pointers)
 * for save-looking strings. It only reads memory, runs once, and sends nothing
 * over the bridge. */
...
static void __cdecl HookLoadCatData(void* self, int64_t key, void* cat_data) {
    ...
    /* TEMPORARY: run the save-path probe on the first load only. */
    if (InterlockedCompareExchange(&g_save_diag_done, 1, 0) == 0) {
        RunSaveLoadDiagnostic(self);
    }
